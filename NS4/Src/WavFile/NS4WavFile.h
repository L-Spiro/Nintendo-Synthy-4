...
namespace ns4 {
	...
	public :
		...
		// == Enumerations.
		/** Formats. */
		enum NS4_FORMAT : uint16_t {
			NS4_F_PCM					= 0x1,
			NS4_F_ADPCM					= 0x2,
			NS4_F_IEEE_FLOAT				= 0x3,
			NS4_F_ALAW					= 0x6,
			NS4_F_MULAW					= 0x7,
			NS4_F_DVI_ADPCM				= 0x11,
			NS4_F_YAMAHA_ADPCM			= 0x16,
			NS4_F_GSM_6_10				= 0x31,
			NS4_F_ITU_G_721_ADPCM		= 0x40,
			NS4_F_MPEG					= 0x50,
			NS4_F_EXTENSIBLE				= 0xFFFE,
		};

		/** Chunks. */
		enum NS4_CHUNKS : uint32_t {
			NS4_C_RIFF					= 0x46464952,
			NS4_C_WAVE					= 0x45564157,
			NS4_C_FMT_					= 0x20746D66,
			NS4_C_DATA					= 0x61746164,
			NS4_C_SMPL					= 0x6C706D73,
			NS4_C_LIST					= 0x5453494C,
			NS4_C_ID3_					= 0x20336469,
			NS4_C_INST					= 0x74736E69,
			NS4_C_INFO					= 0x4F464E49,
			NS4_C_LABL					= 0x6C62616C,
			NS4_C_ADTL					= 0x6C746461,
		};

		/** Metadata. */
		enum NS4_META {
			NS4_M_INAM					= 0x4D414E49,		// Track name.
			NS4_M_IPRD					= 0x44525049,		// Album title.
			NS4_M_IART					= 0x54524149,		// Artist name.
			NS4_M_ICMT					= 0x544D4349,		// Comments.
			NS4_M_ICRD					= 0x44524349,		// Year.
			NS4_M_IGNR					= 0x524E4749,		// Genre.
			NS4_M_ITRK					= 0x4B525449,		// Track number.
			NS4_M_IENG					= 0x474E4549,		// The engineer.
		};


		// == Types.
		/** The save data. */
		struct NS4_SAVE_DATA {
		 ...
		 */
		bool							SaveAsPcm( const char * _pcFolder,
			const char * _pcFileName, const lwaudio &_vSamples,
			const NS4_SAVE_DATA * _psdSaveSettings = nullptr ) const;
		 ...
		 */
		bool							SaveAsPcm( const char * _pcFolder,
			const char * _pcFileName, const lwtrack &_vSamples,
			const NS4_SAVE_DATA * _psdSaveSettings = nullptr ) const {
			lwaudio aAudio;
			aAudio.push_back( _vSamples );
			return SaveAsPcm( _pcFolder,
			_pcFileName, aAudio,
			_psdSaveSettings );
		}
		 ...
		 * \return RETURN
		 */
		inline uint32_t					CalcSize( NS4_FORMAT _fFormat, uint32_t _uiSamples, uint16_t _uiChannels, uint16_t _uiBitsPerPixel ) const {
			switch ( _fFormat ) {
				case NS4_F_PCM : {
		 ...
		 */
		bool							AddListEntry( uint32_t _uiId, const std::string &_sVal );
		 ...
		 */
		const std::vector<NS4_LOOP_POINT> &
										Loops() const { return m_vLoops; }
...
#pragma pack( push, 1 )
		...
		// FMT chunk.
		struct NS4_FMT_CHUNK {
			NS4_CHUNK_HEADER				chHeader;
		...
		struct NS4_SMPL_CHUNK {
			...
			NS4_LOOP_POINT				lpLoops[1];
		};

		// LIST chunk.
		struct NS4_LIST_CHUNK {
			NS4_CHUNK_HEADER				chHeader;
			union {
				char					cName[4];
				uint32_t				uiTypeId;
...
#pragma pack( pop )
		...
		/** Loop points. */
		std::vector<NS4_LOOP_POINT>		m_vLoops;
		/** "LIST" metadata. */
		std::vector<NS4_LIST_ENTRY>		m_vListEntries;
		/** "id3 " metadata. */
		std::vector<NS4_ID3_ENTRY>		m_vId3Entries;
		/** Instrument metadata. */
		NS4_INST_ENTRY					m_ieInstEntry;
		 ...
		 */
		bool							LoadFmt( const NS4_FMT_CHUNK * _pfcChunk );
		 ...
		 * \return Returns true if everything loaded fine.
		 */
		bool							LoadData( const NS4_DATA_CHUNK * _pdcChunk );
		 ...
		 * \return Returns true if everything loaded fine.
		 */
		bool							LoadSmpl( const NS4_SMPL_CHUNK * _pscChunk );
		 ...
		 * \return Returns true if everything loaded fine.
		 */
		bool							LoadList( const NS4_LIST_CHUNK * _plcChunk );
		 ...
		 * \return Returns true if everything loaded fine.
		 */
		bool							LoadId3( const NS4_ID3_CHUNK * _picChunk );
		 ...
		 * \return Returns true if everything loaded fine.
		 */
		bool							LoadInst( const NS4_INST_CHUNK * _picChunk );
		 ...
		 */
		NS4_FMT_CHUNK					CreateFmt( NS4_FORMAT _fFormat, uint16_t _uiChannels, const NS4_SAVE_DATA * _psdSaveSettings ) const;
