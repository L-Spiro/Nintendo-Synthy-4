...
namespace ns4 {
	 ...
	 */
	bool CButterworthFilter::Coefficients( NS4_FILTER_TYPE _ftFilter, const double _dFs, const double _dFreq1_Cutoff, const double _dFreq2_Cutoff, const uint32_t _ui32FilterOrder,
		...
		m_dF1 = 2.0 * std::tan( NS4_PI * m_dF1 / _dFs );
		m_dF2 = 2.0 * std::tan( NS4_PI * m_dF2 / _dFs );
		...
		switch ( _ftFilter ) {
            ...
            case NS4_FT_BANDPASS : {
				ConvertToBandPass();
				break;
			}
			case NS4_FT_BANDSTOP : {
				ConvertToBandStop();
				break;
			}
			default: {
		...
		//nba = 0;
		m_vBa.resize( 2 * max( m_vPoles.size(), m_vZeros.size() ) + 5 );
		...
		if ( _ftFilter == NS4_FT_LOPASS || _ftFilter == NS4_FT_BANDPASS ) {		// Pre-BLT is okay for S-plane.
			m_vBa[0] = m_dPreBltGain * (m_dPreBltGain / m_dGain);				// 2nd term is how much BLT boosts.
		}
		else if ( _ftFilter == NS4_FT_HIPASS || _ftFilter == NS4_FT_BANDSTOP ) {// HF gain != DC gain.
			m_vBa[0] = 1.0 / m_vBa[0];
		}
		...
		if ( _ftFilter == NS4_FT_BANDPASS || _ftFilter == NS4_FT_BANDSTOP ) {
	 ...
	 */
	std::vector<std::complex<double>> CButterworthFilter::PrototypeAnalogLowPass( uint32_t _uiFilterOrder ) {
		std::vector<std::complex<double>> vPoles;
    
		for ( uint32_t I = 0; I < (_uiFilterOrder + 1) / 2; I++ ) {
			double dTheta = (2 * I + 1.0) * NS4_PI / (2 * _uiFilterOrder);
	 ...
	 * \return Returns true.
	 */
	bool CButterworthFilter::Zp2Sos() {
		size_t stFilterOrder = max( m_vZeros.size(), m_vPoles.size() );
	 ...
	 */
	void CButterworthFilter::ConvertToLoPass() {
	 ...
	 */
	void CButterworthFilter::ConvertToHiPass() {
		m_dWc = m_dF2;										// Critical frequency.
	 ...
	 *	Wc^2 = W2 * W1
	 */
	void CButterworthFilter::ConvertToBandPass() {
		m_dBw = m_dF2 - m_dF1;
	 ...
	 *	Wc^2 = W2 * W1
	 */
	void CButterworthFilter::ConvertToBandStop() {
		m_dBw = m_dF2 - m_dF1;
		m_dWc = std::sqrt( m_dF1 * m_dF2 );
